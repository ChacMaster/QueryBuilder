...
&Scoped-Define ENABLED-OBJECTS btnClearDataFilter btnExcel btnText btnHtml ~
btnCsv 
...
DEFINE FRAME frData
     btnClearDataFilter AT Y 41 X 761 WIDGET-ID 76
     ...
     rctDataFilter AT Y 37 X 0 WIDGET-ID 296
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 158 BY 16.33 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U AND VALID-HANDLE(wQuery)
THEN wQuery:HIDDEN = no.
...
&Scoped-define SELF-NAME wQuery
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wQuery wQuery
ON END-ERROR OF wQuery /* Query results */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wQuery wQuery
ON WINDOW-CLOSE OF wQuery /* Query results */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wQuery wQuery
ON WINDOW-RESIZED OF wQuery /* Query results */
DO:
  RUN resizeWindow.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btnCsv wQuery
ON CHOOSE OF btnCsv IN FRAME frData /* CSV */
DO:
  DEFINE VARIABLE i       AS INTEGER   NO-UNDO.
  DEFINE VARIABLE j       AS INTEGER   NO-UNDO.
  DEFINE VARIABLE hColumn AS HANDLE    NO-UNDO EXTENT.
  DEFINE VARIABLE hTable  AS HANDLE    NO-UNDO EXTENT.
  DEFINE VARIABLE hField  AS HANDLE    NO-UNDO EXTENT.
  DEFINE VARIABLE hBuffer AS HANDLE    NO-UNDO.
  DEFINE VARIABLE cField  AS CHARACTER NO-UNDO EXTENT.
  DEFINE VARIABLE iExtent AS INTEGER   NO-UNDO EXTENT.
  
  FOR EACH ttField:
  
  MESSAGE 
  ttField.iColumnNr SKIP
  ttField.hField    SKIP
  ttField.hColumn SKIP
  ttField.hFilter     SKIP
  ttField.lModified       SKIP
  ttField.iExtent 
  ...
  EXTENT(hTable)  = ghDataBrowse:NUM-COLUMNS.
  EXTENT(hField)  = ghDataBrowse:NUM-COLUMNS.
  EXTENT(hColumn) = ghDataBrowse:NUM-COLUMNS.
  EXTENT(cField)  = ghDataBrowse:NUM-COLUMNS.
  EXTENT(iExtent) = ghDataBrowse:NUM-COLUMNS.
  
  DO i = 1 TO ghDataBrowse:NUM-COLUMNS:    
    hColumn[i] = ghDataBrowse:GET-BROWSE-COLUMN(i):HANDLE.
    cField[i]  = ENTRY(NUM-ENTRIES(hColumn[i]:NAME,'.'), hColumn[i]:NAME,'.').
    
    IF NUM-ENTRIES(cField[i],'[') > 1 THEN
      ASSIGN 
        iExtent[i] = INTEGER(ENTRY(1,ENTRY(NUM-ENTRIES(cField[i],'['), cField[i],'[') ,']'))
        cField[i]  = ENTRY(1,cField[i], '[').
    ELSE 
      iExtent[i] = 0.
    
    /* get buffer handle for each column */
    DO j = 1 TO ghDataBrowse:QUERY:NUM-BUFFERS:
      hBuffer = ghDataBrowse:QUERY:GET-BUFFER-HANDLE(j).
      IF hBuffer:NAME = hColumn[i]:TABLE THEN hTable[i] = hBuffer.
    END.
    
    hField[i] = hTable[i]:BUFFER-FIELD(cField[i]):HANDLE.   
  END.  
  
  RUN adf(hColumn, hTable, hField, cField, iExtent).
  
END.

PROCEDURE adf:
  DEFINE INPUT PARAMETER phColumn AS HANDLE    EXTENT.
  DEFINE INPUT PARAMETER phBuffer AS HANDLE    EXTENT.
  DEFINE INPUT PARAMETER phField  AS HANDLE    EXTENT.
  DEFINE INPUT PARAMETER pcField  AS CHARACTER EXTENT.
  DEFINE INPUT PARAMETER piExtent AS INTEGER   EXTENT.
  
  DEFINE VARIABLE i AS INTEGER NO-UNDO.
  
  DO i = 1 TO EXTENT(phColumn):
  
    MESSAGE 
      'column' phColumn[i]:NAME SKIP
      'table'  phBuffer[i]:NAME skip
      'field'  pcField[i] skip
      'extent' piExtent[i] SKIP 
      'value'  phField[i]:BUFFER-VALUE(piExtent[i])
      VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
 ...
 */
  ...
  FOR EACH bField 
    ...
    hField = phQuery:GET-BUFFER-HANDLE(bTable.orderNr):BUFFER-FIELD(cColumnName):HANDLE NO-ERROR.
    ...
    DO:
      ...
      IF ERROR-STATUS:ERROR THEN hField:FORMAT = bField.fieldFormat NO-ERROR. /* Restore original format */
    END.
    ...
    hField:PRIVATE-DATA = STRING(bField.hFilter).
...
------------------------------------------------------------------------------*/
  ENABLE btnClearDataFilter btnExcel btnText btnHtml btnCsv 
      WITH FRAME frData IN WINDOW wQuery.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    RUN buildQuery(OUTPUT hQuery).
    gcBaseQuery = hQuery:PREPARE-STRING.
    
    RUN buildBrowse(hQuery).
    RUN openQuery(hQuery).
